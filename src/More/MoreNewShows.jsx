...
const MoreNewShows = () => {
    ...
    const selectionResult = async() => {
        try{
          ...
          console.log(document.querySelector(".D").scrollHeight);
        }catch{
          console.log(Error);
        }
    }
    ...
    useEffect(() => {
        getmovies7(`https://api.themoviedb.org/3/tv/popular?language=en-US&page=${pageCount}&api_key=c52e8c9749d8e16e1e525525da148b63`);
    },[pageCount]);
        ...
        <div className="row">
          {discover.map((movie)=>
            <div className="col" key={movie.id}>
                ...
                <a style={{textAlign:''}} href="http://" id={movie.id} onClick={pickedMovie}>{movie.name}<br></br> <span>Started In ({movie.first_air_date.slice(0,4)})</span></a>
            </div>  
            )}
        </div>
        <br />
